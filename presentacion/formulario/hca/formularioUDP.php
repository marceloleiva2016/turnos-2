...
$hca = new Hca($hcaId);
if ($hca->tipoHCA != TipoHCA::UDP) {
	throw new Exception('El ID de HCA no corresponde a una UDP.', 20121113164200);
}

$hca->cargarTodoHCA(); 
$paciente = $hca->internacion->paciente;
$hayEgreso = $hca->hayEgresoUDP();
...
<form action="formularioUDP.php">	
	...
	<div id="contenedorMenuOpciones">
		...
		<div id="menuOpciones">
			<ul>
				...
				<li><a href="#" id="btnSalirDeUDP">Salida UDP</a></li>
				<li><a href="#" id="btnImprimir">Imprimir </a></li>
				<li><a href="#" id="btnCmbPaciente">Cambiar Paciente</a></li>
			</ul>
		</div>
	...
	<div id="contenedor">
		<div id="tituloImprimir" class="cnt">
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
			  <tr>
			    ...
			    <td align="right" width="70"><b><?php echo $hca->getNombreSector();?></b></td>
			  </tr>
			  <tr>
		...
		<div id="contenedorSalidaHS" class="cnt" >
			...
			<thead>
			<tr>
				<th style="border-bottom: solid; border-bottom-width: 1pt; text-align: left;">Datos del Egreso (UDP)</th>
			</tr>
			</thead>
			<tbody>
			<?php 
				if ($hca->hayEgresoUDP()) { 
					//TODO: si el usuario esta en la lista de los permitidos
			...
			?>
					<table width="100%">
						<tr>
							<td><b>Fecha:</b>
								<?php
									print Utils::phpTimestampToHTMLDate($hca->salidaUDP->fecha);
								?>
							</td>
							<td>&nbsp;</td>							
							<td><b>Profesional:</b>
								<?php 
									print Utils::phpStringToHTML($hca->salidaUDP->nombreProfesional());
								?>
							</td>
						</tr>
						<tr>
							...
							<b>Diag. Egreso:</b>
								...
								<?php 
									echo "<span class=\" context-menu-one-egreso box menu-1\" diagId=\"".$hca->salidaUDP->id."\" diagUsr=\"".$hca->salidaUDP->usr."\">\n";
									print Utils::phpStringToHTML($hca->salidaUDP->diagnosticoToString());
									echo "</span>\n"; 
								?>
							</td>
						</tr>
						<tr>
							<td colspan="2"><b>Destino:</b>
								<?php 
									print Utils::phpStringToHTML($hca->salidaUDP->destinoToString());
								?>
							</td>
							<td>
								<?php 
									if ($hca->salidaUDP->destino == TipoDestinoUDP::DERIVACIONint) {
										$centro = $gralesDBLinker->nombreCentro($hca->salidaUDP->centro);
										print "<b>Centro:</b> " . Utils::phpStringToHTML($centro);
									} elseif ($hca->salidaUDP->destino == TipoDestinoUDP::INTERNACION) {
										$nombre = $gralesDBLinker->nombreEspecialidad($hca->salidaUDP->servicio);
...
<!-- El siguiente DIV es donde se muestra la lista de pacientes en UDP -->
<div id="toolbar-container" style="display:none;">
</div>

<form action="formularioUDP.php" method="post" id="formHCA" onsubmit="javascript:SaveScrollPosition()">
