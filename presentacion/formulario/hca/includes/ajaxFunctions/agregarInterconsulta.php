...
{
	$data->result = false;
	$data->message = "Se debe cargar una interconsulta";
}
else 
{
...
$inter = new Interconsulta($descripcion);

if($data->result)
{
	try {
		$hcaDb->insertarInterconsulta($idHCA, $inter, $user_id);	
	} catch (Exception $e) {
		$data->result = false;
		$data->message = "No se pudo agregar la interconsulta: " . $e->getMessage();
	}
}
...
if($data->result)
{
	...
	$data->message = "Interconsulta agregada correctamente";
}
