...
{
	$error =$error|| $base->hayEgresoHSDAP($idHCA);
	if($error)
	{
		$message = "Ya existe una salida de DAP";
	}
}
?>
...
{
?>
	<form action="salidaDAP.php" id="formDatos">
	<div id="divPrincipal" title="Salida DAP" style="width: 500px; margin: 0 auto 0 auto">
		...
		<b>
		   		...
		   		<?php
		   			$destinos = $base->listaDestinosHSDAP();
		   			foreach ($destinos as $key => $value) {
		   				echo "<input type=\"radio\" name=\"destino\" value = \"$key\" " .(($key==1)?"CHECKED":""). " >$value</input>";
		   				if($key==TipoDestinoHS::DERIVACIONint)
		   				{
		   				...
		   				if($key==TipoDestinoHS::INTERNACION)
		   				{
